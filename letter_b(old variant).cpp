...
const int nGridL = 128;
const int nGridX = 32;
const int numVertices = ( nGridL + 6 ) * ( nGridX + 6) * 6;

const float l1=2;		// длина верхней детали тв. знака
const float l2=3;		// длина вертикальной прямой детали
const float R=1;		// радиус закругления
const float r=0.5f;		// радиус полуцилиндра

// длина "распрямленной буквы"
...
void center (float l, float *result)
{
    // Нахождение координат центра текущего сечения
	float yc, zc;
...
D3DXVECTOR3 rect_to_surf ( float l, float x, D3DXVECTOR3 *p)
{
	...
	// координаты вектора, перпендикулярного к плоскости сечения
	A = res[0] - x1;
	B = res[1] - y1;
	C = res[2] - z1;
	...
	// Нормаль данной точки
	*p = D3DXVECTOR3((x-x1)/r,(y-y1)/r,(z-z1)/r);

	
	//D3DXMatrixRotationYawPitchRoll( &rotGen, rotY, rotX, rotZ );
	D3DXMatrixTranslation( &rotGen, 3, 3, 3);
